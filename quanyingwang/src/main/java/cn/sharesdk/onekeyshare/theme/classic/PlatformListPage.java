...
public class PlatformListPage extends PlatformListFakeActivity implements View.OnClickListener {
	...
	private void initPageView() {
		...
		llPage.setBackgroundDrawable(new ColorDrawable(0xfff4f4f4));
		...
		btnCancel.setTextColor(0xff252525);
		btnCancel.setTextSize(16);
		int resId = getStringRes(getContext(), "ssdk_oks_cancel");
		if (resId > 0) {
			btnCancel.setText(resId);
		}
		int dp = com.mob.tools.utils.R.dipToPx(getContext(), 12);
		btnCancel.setPadding(dp, dp, dp, dp);

		resId = getBitmapRes(getContext(), "ssdk_oks_classic_platform_corners_bg");
		if (resId > 0) {
			btnCancel.setBackgroundResource(resId);
		} else {
			btnCancel.setBackgroundDrawable(new ColorDrawable(0x00ffffff));
		}
